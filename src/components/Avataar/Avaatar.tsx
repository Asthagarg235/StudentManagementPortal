import { DetailedHTMLProps, HTMLAttributes } from "react";
import {memo} from "react";
interface Props{
    totalElement: number;
}
const Avaatar: React.FC<Props> = ({totalElement}) => {
    const names = ['https://designreset.com/cork/ltr/demo4/assets/img/profile-12.jpeg',
     'https://designreset.com/cork/ltr/demo4/assets/img/profile-12.jpeg', 
     'data:image/png;base64,']
    if(totalElement==NaN){
        var ele = false;
    }
    else if(totalElement<=names.length) {
        var ele = false;
    }
    else{
        var ele = true;
    }
     return (
    <div className="flex flex-row -space-x-4">
    <div className="w-16 h-16 ml-10 display: inline-block position: relative">
        <img src={names[0]} className="rounded-full"></img>
    </div>
    <div className="w-16 h-16  display: inline-block position: relative ml-n2 mr-n2">
      <img src={names[1]} className="rounded-full" ></img>
    </div>
    <div className="w-14 h-14 display: inline-block position: relative">
      <img src={names[2]} className="rounded-full"></img>
    </div>
    <div className="display: inline-block position: relative text-center">
        {ele && <h2>+{totalElement-names.length} more</h2>}
    </div>
  </div>
);
};

Avaatar.defaultProps = {
}

export default memo(Avaatar);